#!/usr/bin/env bash
# run-benchmark.sh — Benchmark Huihui-CyberStrike with different expert routing (top-k)
#
# Usage:
...
start_vllm() {
    ...
    vllm serve "$model_dir" \
        --served-model-name "huihui-cyberstrike" \
        --port "$VLLM_PORT" \
        --host 0.0.0.0 \
...
# ── Main ──
main() {
    log "=== Huihui-CyberStrike MoE Benchmark ==="
