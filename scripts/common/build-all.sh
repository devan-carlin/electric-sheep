...
#
# Usage:
#   ./build-all.sh              # Interactive (asks what to build)
#   ./build-all.sh --all        # Build everything
#   ./build-all.sh --vllm       # Build vLLM only
#   ./build-all.sh --llama      # Build llama.cpp only
#   ./build-all.sh --prereqs    # Install prerequisites only
#   ./build-all.sh --status     # Show current build state
# ============================================
...
UBUNTU_SCRIPTS="$SCRIPTS/ubuntu"
CONFIGS="$HOME/electric-sheep/configs"
...
show_status() {
    ...
    for s in "$UBUNTU_SCRIPTS"/0*.sh; do
        if [ -f "$s" ]; then
            echo "  $(basename "$s")"
        fi
    done
...
# ============================================
build_prereqs() {
    run_script "$UBUNTU_SCRIPTS/01-install-prerequisites.sh" "Prerequisites"
}

build_vllm() {
    # Setup project directory + venv
    run_script "$UBUNTU_SCRIPTS/02-setup-project-directory.sh" "vLLM Project Setup"

    # Build vLLM XPU
    run_script "$UBUNTU_SCRIPTS/03-build-vllm-xpu.sh" "vLLM XPU Build"

    # Patch MoE qzeros (post-build fix)
    run_script "$UBUNTU_SCRIPTS/03.1-patch-vllm-moe-qzeros.sh" "vLLM MoE qzeros Patch"

    echo ""
    ok "vLLM build pipeline complete!"
    echo ""
    echo "  Next: download models"
    echo "  bash $UBUNTU_SCRIPTS/06-download-models.sh"
}

build_llama() {
    # Setup project + clone source
    run_script "$UBUNTU_SCRIPTS/04-setup-llama.sh" "llama.cpp Project Setup"

    # Build
    run_script "$UBUNTU_SCRIPTS/05-build-llama-cpp.sh" "llama.cpp SYCL Build"
...
build_both() {
    ...
    echo ""
    echo "  Next steps:"
    echo "  - Download models:  bash $UBUNTU_SCRIPTS/06-download-models.sh"
    echo "  - Start vLLM:       source $VLLM_DIR/.venv/bin/activate"
    echo "  - Start llama.cpp:  bash $CONFIGS/llama/deepseek/start-deepseek-v4-flash.sh"
}

download_models() {
    run_script "$UBUNTU_SCRIPTS/06-download-models.sh" "Download Models"
}
