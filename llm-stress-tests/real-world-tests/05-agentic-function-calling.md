# Test 5: Tool Use & Agentic Planning — Simulated Function Calling

**Target Capability:** Multi-step orchestration and structured payload generation.

**Why it's a great test:** If you want to use your local LLM for agentic workflows (like AutoGen, LangChain, or Ollama function calling), it needs to know *when* and *how* to trigger tools.

**The Task:** Provide the model with documentation for 4 hypothetical REST API tools. Give it a complex user request. Ask the LLM to output a sequential JSON execution plan array containing the exact order of tool calls and parameters required to complete the task.
