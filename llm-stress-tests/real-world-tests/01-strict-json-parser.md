# Test 1: Strict Output Compliance — Messy Log to JSON Parser

**Target Capability:** Deterministic formatting, edge-case handling, zero conversational fluff.

**Why it's a great test:** Small and medium local models frequently fail when asked to adhere *strictly* to a JSON schema without adding explanatory text like *"Here is your JSON:"* or leaking invalid syntax when encountering weird input data.

**The Task:** Feed the LLM a block of intentionally messy, unstructured text (e.g., raw server logs, unformatted customer support emails, or scraped dirty HTML). Instruct it to output **only valid, raw JSON** conforming to a specific schema (keys, array types, boolean flags).

**Stress Factor:** Include edge cases in the prompt—like missing fields, double quotes inside strings, or dates in different timezones—and see if it handles `null` values or standardizes formats correctly.
